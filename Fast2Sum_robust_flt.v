...
apply/round_le.
apply/(Rle_trans _ ( succ beta fexp a )); try lra.
apply/hh.
apply/generic_format_pred.
apply/generic_format_pred.
  apply/generic_format_FLT_bpow; lia.
...
rewrite Rabs_pos_eq; move:(bpow_gt_0 beta (fexp ( t  + p))); lra.
...
apply/bpow_le.
move:uxt; rewrite ulp_neq_0; last lra.
move/bpow_inj; rewrite /cexp /fexp; lia.
by apply/bpow_mag_le.
...
ring_simplify(t - k + (k - t))%Z; rewrite pow0 Rmult_1_r.
move => hmb.
...
move:(bpow_gt_0 beta k); nra.
Qed.


Lemma fastTwoSum_correc_mag' a b (Fa: format a) (Fb : format b) rnd 
                   (valid_rnd: Valid_rnd rnd ) (rndD: round_direct rnd) :
  (a <> 0 -> (mag beta  b <= mag beta  a)%Z)-> (mag beta  a <=  mag beta b + p)%Z ->
let h := round beta fexp rnd (a + b) in 
let z := round beta fexp rnd (h - a) in 
let l := round beta fexp rnd (b - z) in 
  h + l = (a + b).
...
wlog apos: a b Fa Fb  blea rnd valid_rnd rndD/ 0 <= a.
  ...
  + by apply/generic_format_opp.
  +  by apply/generic_format_opp.
  + rewrite !mag_opp =>h; apply/blea; lra.
  + by right.
  + rewrite /ma ;lra.
  + by rewrite !mag_opp.
 + by apply/generic_format_opp.
...
case:apos=> [apos| <-]; last first.
  rewrite /=  Rplus_0_l Rminus_0_r !(round_generic _ _ _ b Fb) Rminus_eq_0
    round_0 =>*; ring.
have {} blea:  (mag beta b <= mag beta  a)%Z by apply/blea; lra.
move=> maga_le_magb.
case:(Req_dec b 0)=>[/= ->|b_neq0].
by rewrite !(Rsimp01, (round_generic _ _ _ a), round_0, Rminus_eq_0).

rewrite /=.
have ->: round beta fexp rnd (round beta fexp rnd (a + b) - a) = 
  (round beta fexp rnd (a + b) - a). 
  rewrite round_generic //.
  apply/sma_exact =>//.
  by rewrite /cexp;apply/FLT_exp_monotone;  lia.
...
wlog hwlog: a b Fa Fb  blea b_neq0 apos maga_le_magb / pow (p - 1)  <= a < pow p; last first.
rewrite -[RHS]Fast2Sum_correct_aux //.
have zE: round beta fexp rnd (round beta fexp rnd (a + b) - a) = 
  (round beta fexp rnd (a + b) - a). 
  rewrite round_generic //.
  apply/sma_exact =>//.
  by rewrite /cexp;apply/FLT_exp_monotone;  lia.
...
case:(Req_dec (a + b) 0)=>[ab0 /=|abneq0].  
  rewrite ab0 round_0 Rminus_0_l Rplus_0_l.
    have->: b - - a= a + b by ring.
    by rewrite ab0 round_0.
case: (Rle_lt_dec  (Rabs (a + b))  (pow (emin + p)))=> Hapb.
  have Fab: format (a + b) by apply/Hauser.
  have hE:  round beta fexp rnd (a + b) = a+b.
    by rewrite  round_generic.
  rewrite hE ;ring_simplify(a+b -a); rewrite  round_generic.
    by ring. 
  by rewrite Rminus_eq_0 ; apply/generic_format_0. 
have magabgt: (emin + p < mag beta (a + b))%Z by apply/mag_gt_bpow; lra.
set h := round _ _ _ (a + b).
set z := h -a; set l := round _ _ _ (b -z).
have Fz: format z by apply/sma_exact.
case: (Rle_lt_dec  (Rabs (b - z ))  (pow (emin + p)))=> Hbmz.
  have Fab: format (b + (-z)).
    apply/Hauser=>//.
    by apply/generic_format_opp.
  rewrite /l round_generic //; rewrite /z;lra.
have magabzgt: (emin + p < mag beta (b - z ))%Z by apply/mag_gt_bpow; lra.
 have xS: forall x, 
    x  = ( x * (pow (p - (mag beta a))) * (pow ((mag beta a ) - p))).
    move=> x; rewrite Rmult_assoc -bpow_plus -[LHS]Rmult_1_r.
    congr Rmult; have ->: 1 = pow 0 by [].
    by congr bpow; lia.

have abin:  pow (p - 1) <= a *  (pow (p - (mag beta a))) < pow p .
  suff :  pow (p - 1) * pow ( mag beta a - p) <= 
          a * (pow (p - (mag beta a))) * pow ( mag beta a - p) 
          < pow p  * pow ( mag beta a - p) .
    by move: (bpow_gt_0 beta ( mag beta a - p) ); nra.
  rewrite -(xS a) -!bpow_plus.
  ring_simplify(p - 1 + (mag beta a - p))%Z; ring_simplify (p + (mag beta a - p))%Z.
  rewrite -{3 4} (Rabs_pos_eq a); last lra.
  by case:mag=>//= maga; apply; lra.
have h1:  
((mag beta (b * pow (p - mag beta a)) <= mag beta (a * pow (p - mag beta a))))%Z.
rewrite !mag_mult_bpow; try lra; lia.
have b'neq0:  b * pow (p - mag beta a) <> 0 .
    by move: (bpow_gt_0 beta ( p - mag beta a ) ); nra.
have a'pos: 
   0 < a * pow (p - mag beta a) by move: (bpow_gt_0 beta ( p - mag beta a ) ); nra.

have h2 :   (mag beta (a * pow (p - mag beta a)) <= mag beta (b * pow (p - mag beta a)) + p)%Z.
by rewrite !mag_mult_bpow; try lra; lia.

(* rewrite /= (round_generic _ _ _ (_ - a)); last first. *)
(*  apply/sma_exact=>//. *)
case:(Z_lt_le_dec p (mag beta a))=>[pgtmaga|magagep]; last first.
  have Fa' : format  (a * pow (p - mag beta a)).
    by apply/mult_bpow_pos_exact_FLT=>//; lia.
  have Fb' : format  (b * pow (p - mag beta a)).
    by apply/mult_bpow_pos_exact_FLT=>//; lia.
  apply/(Rmult_eq_reg_r (pow (p - mag beta a)));
  last by move:(bpow_gt_0 beta (p - mag beta a)); lra.
  rewrite !Rmult_plus_distr_r.
  rewrite -[RHS]Hwlog=>//.

(* rewrite /z (round_generic _ _ _ (_ - a)); last first. *)
(*  apply/sma_exact=>//. *)
(* rewrite (round_generic _ _ _ (_ - a * _)); last first. *)
(*  apply/sma_exact=>//. *)
(* rewrite /cexp;apply/FLT_exp_monotone;  lia. *)
  congr Rplus.
    by rewrite -Rmult_plus_distr_r round_bpow_flt //; lia.
  set RND:= round _ _ _.
  rewrite -Rmult_plus_distr_r.
  rewrite /RND !(round_bpow_flt _ (a+b) (p -mag beta a)); last lia.
  rewrite -!Rmult_minus_distr_r.
  by rewrite round_bpow_flt_pos //  -/h -/z; lia.

(* p < mag beta a *)
have magbge1: ( 1 <= mag beta b)%Z by lia.
have blB: 1 <= Rabs b.
  apply/(Rle_trans _ (pow (mag beta  b - 1))); last by apply/bpow_mag_le.
  by rewrite -pow0 ; apply/bpow_le; lia.

have Fa' : format  (a * pow (p - mag beta a)).
  apply/mult_bpow_exact_FLT =>//; lia.
have Fb' : format  (b * pow (p - mag beta a)).
  by apply/mult_bpow_exact_FLT=>//; lia.
case : (Req_dec (b - z)  0)=>[leq0 | lneq0].
  rewrite /l leq0 round_0.
  by move: leq0; rewrite /z; lra.
...
move:( Hwlog (a * pow (p - mag beta a)) (b * pow (p - mag beta a)) Fa' Fb' h1 b'neq0 a'pos h2 abin).
rewrite /=.
set h' := round _ _ _ _.
set l' := round _ _ _ _.


rewrite  -(Rmult_plus_distr_r _ _ (pow _ )).
move=> <-.
(* ici *)
case:(Z_lt_le_dec  (mag beta (a + b))  (emin + mag beta a))=> eminmagab.
  have Fh': format  (a * pow (p - mag beta a) + b * pow (p - mag beta a)).
    apply/Hauser.
    - apply/mult_bpow_exact_FLT=>//; lia.
    - apply/mult_bpow_exact_FLT=>//; lia.
    rewrite -Rmult_plus_distr_r Rabs_mult.
    rewrite (Rabs_pos_eq  (pow _)); last by apply/bpow_ge_0.
    apply/(Rmult_le_reg_r (pow (mag beta a -p))); first by apply/bpow_gt_0.
    rewrite Rmult_assoc -!bpow_plus.
    ring_simplify(p - mag beta a + (mag beta a - p))%Z.
    rewrite pow0 Rmult_1_r.
    ring_simplify  (emin + p + (mag beta a - p))%Z.
    apply/(Rle_trans _ (pow (mag beta (a + b)))).
      by apply/Rlt_le/bpow_mag_gt.
    apply/bpow_le; lia.
  have Fab: format (a + b).
    have-> : (a + b) = 
   (a * pow (p - mag beta a) + b * pow (p - mag beta a)) * 
   (pow (mag beta a - p)).
      rewrite -Rmult_plus_distr_r Rmult_assoc  -!bpow_plus.
      ring_simplify (p - mag beta a + (mag beta a - p))%Z.
      by rewrite pow0 Rmult_1_r.
    by apply/mult_bpow_pos_exact_FLT; last lia.
  have l0: l = 0.
    rewrite /l  /z /h (round_generic _ _ _ (a +b)) //.
    have->: a + b - a = b by ring.
    by rewrite Rminus_eq_0 round_0.
  have l'0 : l' = 0.
   have r0:= (round_0 beta fexp rnd).
   rewrite -r0 /l'; congr round.
   rewrite /h' (round_generic _ _ _ (_ + _)) //.
   ring_simplify     (a * pow (p - mag beta a) + b * pow (p - mag beta a) -
     a * pow (p - mag beta a)).
    by lra.
   have ->: h' =  (a * pow (p - mag beta a) + b * pow (p - mag beta a)).
     by rewrite /h' round_generic.
   by rewrite /h  round_generic // l0 l'0 ; ring.

rewrite /h' -Rmult_plus_distr_r round_bpow_flt; last lia.
rewrite Rmult_plus_distr_r; congr Rplus.
rewrite /l /z.
(* ici *)
 case:(Rle_lt_dec ( pow (emin + mag beta a - 1))  (Rabs (b - (h - a))))=> hh.



rewrite /l'.

have ->: h' = h *  pow (p - mag beta a).
by rewrite /h' -Rmult_plus_distr_r round_bpow_flt //;lia.

rewrite -!Rmult_minus_distr_r round_bpow_flt //.
rewrite Z.min_l; last lia.
ring_simplify (mag beta (b - (h - a)) + (p - mag beta a) - p)%Z.


suff : (emin + mag beta a   <= mag beta (b - ( h - a)))%Z by lia.




apply/mag_ge_bpow.
case:(ex_shift beta fexp a (emin + mag beta a - 1))=>//.

rewrite /cexp /fexp ;lia.
rewrite -round_bpow_flt;last first.
ring_simplify(mag beta (b - (h - a)) + (p - mag beta a) - p)%Z.
rewrite Z.min_l; last lia.
...
move=> ma aE.
have magh: (emin + mag beta a <= mag beta h)%Z.
  apply/(Z.le_trans _ (mag beta (a + b)))=>//.
  apply/mag_round_ge ; by apply/round_plus_neq_0.
(*ici *)


case:(ex_shift beta fexp h (emin + mag beta a - 1))=>//.
...
ex_shift
     : forall (beta : radix) (fexp : Z -> Z) (x : R) (e : Z),
       ...
       (e <= cexp beta fexp x)%Z -> exists m : Z, x = IZR m * bpow beta e
suff : pow (emin + mag beta a - 1 ) <= Rabs (b - (h - a)).

apply/mag_ge_bpow.
have Fh : format h by apply/generic_format_round.
...
have ab_imul: is_imul h (  pow (emin + mag beta a - 1)).



apply/(is_imul_format_mag_pow   Fh).
rewrite /fexp ;try lia.
rewrite Z.max_l ; last  lia.
...
mag_round_ge:
  ...
  (mag beta x <= mag beta (round beta fexp rnd x))%Z

is_imul_pow_le:
  forall [beta : radix] [x : R] [y1 y2 : Z],
  is_imul x (bpow beta y1) -> (y2 <= y1)%Z -> is_imul x (bpow beta y2)
is_imul_pow_round:
  forall (p emin : Z) [beta : radix] [rnd : R -> Z],
  Valid_rnd rnd ->
  forall (x : R) (y : Z),
  is_imul x (bpow beta y) ->
  is_imul (round beta (FLT_exp emin p) rnd x) (bpow beta y)
is_imul_format_mag_pow:
  forall [p emin : Z] [beta : radix] [x : R] [y : Z],
  generic_format beta (FLT_exp emin p) x ->
  (y <= FLT_exp emin p (mag beta x))%Z -> is_imul x (bpow beta y)
is_imul_pow_mag:
  forall [beta : radix] [x : R] [y : Z],
  x <> 0 -> is_imul x (bpow beta y) -> (y <= mag beta x - 1)%Z




mag_ge_bpow:
  forall (r : radix) (x : R) (e : Z),
  bpow r (e - 1) <= Rabs x -> (e <= mag r x)%Z
emin := (3 - emax - p)%Z.

omega := (1 - pow (-p)) * pow emax.

Lemma fastTwoSum_correct a b (Fa: format a) (Fb : format b) rnd 
                   (valid_rnd: Valid_rnd rnd ):
  (a <> 0 -> Rabs b <= Rabs a) ->
let h := round beta fexp rnd (a + b) in 
let z := round beta fexp rnd (h - a) in 
